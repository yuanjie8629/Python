...
def knapsack(capacity: int, weightage: List[int], value: List[int], count: int) -> int:
    ...
    """
    This algorithm will return the maximum value of object that can be put in
    a knapsack with a given capacity cap,
    ...
    # Return info (whether nth item is included or not)
        ...
        return max(
            value[count - 1]
            + knapsack(capacity - weightage[count - 1], weightage, value, count - 1),
